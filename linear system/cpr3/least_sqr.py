...
# there is a better description of how i handle my data in the description word file,
# if you want to add more data read it.
start = 1950
years = []
for ind in range(73):
    years.append([ind])
...
# i just used this method to see how close to it qr factorization would get
def estimate(x,equ):
    a = x - 1950
...
# q,r are of modified gram-schmidt
# q_h,r_h are householder
q,r = QR_fac(years[50:])
...
# making prediction by using calculated x
def estimate_gram(num):
    num = num - 2000
    # change through x_h and x if you want to switch up between householder and gram-qr
    return [[num]] @ x 
def estimate_hous(num):
    num = num - 2000
    return [[num]] @ x_h

a = int(input('input year on which you wish to make an estimation : '))
ws = input('type 1 if you want to use gram-schmidt, other for householder : ')
if(ws == '1'):
    print(estimate_gram(a))
else:
    print(estimate_hous(a))